...
$report(str_contains($dashboard, 'oneid-user-session.js?v=20260916-1') && str_contains($dashboard, 'oneid-user-session.css?v=20260916-1'), 'user dashboard loads cache-busted controller assets');
...
$report(str_contains($security, 'oneid-user-session.js?v=20260916-1') && str_contains($security, 'OneIdUserSessionConfig'), 'authenticated Account Security page loads user controller');
...
$report(
    str_contains($controller, 'function decorateRenewalDialog()')
        && str_contains($controller, 'window.setTimeout(decorateRenewalDialog, 0)')
        && substr_count($controller, 'window.setTimeout(markTerminalDialog, 0)') === 1
        && str_contains($controller, "confirmButton.addEventListener('click', clearRenewalDialogState"),
    'successful renewal uses a non-terminal dialog and explicitly clears its overlay'
);
$report(
...
$report(str_contains($css, 'max-height: calc(100dvh - 32px)') && str_contains($css, 'backdrop-filter: none'), 'mobile renewal dialog remains in the viewport without an expensive blur layer');
