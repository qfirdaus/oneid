...
final class SsoConfigurationService
{
    ...
    public function history(int $page,int $pageSize): array
    {
        ...
        $result=method_exists($this->operation,'configuration_audit_list')?$this->operation->configuration_audit_list($page,$pageSize):$this->operation->configuration_history_list($page,$pageSize);$rows=[];
        foreach($result['rows']??[] as$row){$rows[]=['id'=>(string)($row['audit_id']??$row['history_id']),'version_before'=>$row['configuration_version_before']===null?null:(int)$row['configuration_version_before'],'version_after'=>$row['configuration_version_after']===null?null:(int)$row['configuration_version_after'],'actor'=>(string)$row['actor_id'],'action'=>(string)$row['action_name'],'outcome'=>(string)$row['outcome'],'reason_code'=>(string)$row['reason_code'],'change_reason'=>$row['change_reason'],'before'=>$row['before_json']?json_decode($row['before_json'],true):null,'after'=>$row['after_json']?json_decode($row['after_json'],true):null,'correlation_id'=>(string)$row['correlation_id'],'created_at'=>(string)$row['created_at']];}
