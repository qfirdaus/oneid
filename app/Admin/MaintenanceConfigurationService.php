...
final class MaintenanceConfigurationService
{
    ...
    public function update(array $post,string $admin,string $ip): array
    {
        ...
        $started=false;try{$this->operation->beginTransaction();$started=true;$before=$this->operation->get_maintenance_config_for_update();if(!is_array($before)||(int)$before['configuration_version']!==(int)$version)throw new SsoConfigurationException('MT3_CONFIGURATION_STALE',$cid);if($this->operation->update_maintenance_config_by_version((int)$before['id'],$after,(int)$version)!==1)throw new SsoConfigurationException('MT3_UPDATE_FAILED',$cid);$wasActive=(bool)(MaintenancePolicy::evaluate($before)['active']??false);$willBeActive=(bool)(MaintenancePolicy::evaluate($after)['active']??false);if(!$wasActive&&$willBeActive){if(!method_exists($this->operation,'admin_step_up_revoke_all_access_grants_for_maintenance'))throw new SsoConfigurationException('MT3_GRANT_REVOCATION_UNAVAILABLE',$cid);$this->operation->admin_step_up_revoke_all_access_grants_for_maintenance();}$next=(int)$version+1;$this->operation->configuration_history_record(['version_before'=>$version,'version_after'=>$next,'actor_id'=>$admin,'ip_address'=>$ip,'action_name'=>'UPDATE_MAINTENANCE_CONFIGURATION','outcome'=>'SUCCESS','reason_code'=>'MT3_UPDATED','change_reason'=>$reason,'before'=>MaintenancePolicy::evaluate($before),'after'=>$after,'correlation_id'=>$cid]);if($this->operation->syslog_record(69,"admin=$admin action=maintenance_update mode=$mode correlation=$cid",$ip)!==1)throw new SsoConfigurationException('MT3_AUDIT_FAILED',$cid);$notificationId=$this->notification===null?null:($this->notification)('MAINTENANCE_CHANGED',$admin,$cid,$cid,['Mode'=>$mode,'Valid from'=>(string)($starts??'-'),'Valid until'=>(string)($ends??'-'),'Reference'=>$cid]);$this->operation->commit();$started=false;return['status'=>1,'code'=>'MT3_UPDATED','data'=>MaintenancePolicy::evaluate($after+['configuration_version'=>$next]),'notification_queued'=>$notificationId!==null,'correlation_id'=>$cid];}catch(Throwable $e){if($started)try{$this->operation->rollback();}catch(Throwable){}if($e instanceof SsoConfigurationException)throw $e;throw new SsoConfigurationException('MT3_UPDATE_FAILED',$cid);}
    }
}
