...
final class SessionHistoryService
{
    ...
    public function list(array $input):array
    {
        ...
        $date=function(string $value,string $fallback,bool $end):string{$value=trim($value);if($value==='')return$fallback;$d=\DateTimeImmutable::createFromFormat('Y-m-d',$value);if(!$d||$d->format('Y-m-d')!==$value)throw new InvalidArgumentException('SH1_DATE_INVALID');return$value.($end?' 23:59:59':' 00:00:00');};
        $result=$this->operation->admin_list_session_history(['page_size'=>$size,'offset'=>($page-1)*$size,'query'=>$query,'reason'=>$reason,'from'=>$date((string)($input['date_from']??''),'1970-01-01 00:00:00',false),'to'=>$date((string)($input['date_to']??''),'2999-12-31 23:59:59',true)]);
        ...
        $total=(int)$result['total'];return['status'=>1,'code'=>'SH1_HISTORY_LOADED','data'=>$rows,'meta'=>['page'=>$page,'page_size'=>$size,'total'=>$total,'total_pages'=>max(1,(int)ceil($total/$size)),'query'=>$query,'reason'=>$reason]];
    }
}
