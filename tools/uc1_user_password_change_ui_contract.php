...
$check(!str_contains($ui,'Sekurang-kurangnya 8 aksara')&&str_contains($ui,"oneid_translate('dashboard.password.length')")&&str_contains($ui,'minlength="12"'),'localized initial UI copy matches the 12-character server policy');
...
$check(str_contains($ui,"complete: function(){")&&str_contains($ui,"dashboardI18n.passwordHttpFailed.replace('{status}',xhr.status)"),'localized transport failure is visible and controls are restored');
