...
$report(str_contains($database, 'WHERE id = :config_id AND singleton_key = 1') && str_contains($database, 'PDO::PARAM_INT'), 'persistence update targets the identified singleton row');
...
$rows = $pdo->query('SELECT id,singleton_key,token_timeout,multi_session,password_reset_email_enabled FROM sys_config')->fetchAll(PDO::FETCH_ASSOC);
$report(count($rows) === 1 && (int) $rows[0]['singleton_key'] === 1 && (string) $rows[0]['token_timeout'] === '0.5' && (int) $rows[0]['multi_session'] === 1 && (int) $rows[0]['password_reset_email_enabled'] === 1, 'live UAT singleton row exists and policy values remain at baseline');
