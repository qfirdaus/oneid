...
'production_maker_checker'=>str_contains($service,"\$target==='EMERGENCY_BYPASS'&&\$duration>120")&&str_contains($service,'USER_MFA_APPROVAL_SELF_REJECTED')&&str_contains($service,"'PENDING_APPROVAL'"),
'production_single_admin_bounded'=>str_contains($service,'if (!$requiresApproval)')&&str_contains($js,"requiresApproval()?'Submit for approval':'Apply change'"),
...
'mode_selection_guidance'=>str_contains($ui,'user_mfa_mode_hint')&&str_contains($ui,'user-mfa-mode-hint--full')&&str_contains($js,'modeHints')&&str_contains($js,'Penguatkuasaan penuh')&&str_contains($js,'User MFA dimatikan sepenuhnya')&&str_contains($js,'Maintenance MFA dan Admin Step-Up tidak dimatikan'),
