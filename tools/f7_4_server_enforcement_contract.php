...
$guard=(string)file_get_contents(dirname(__DIR__).'/lib/request_security.php');$dashboard=(string)file_get_contents(dirname(__DIR__).'/admin/dashboard.php');$users=(string)file_get_contents(dirname(__DIR__).'/admin/user_list.php');$tests['ajax_guard_wired']=str_contains($guard,"oneid_require_admin_step_up(\$operation,oneid_admin_action_purpose(\$matchedActions[0]),true)");$tests['pages_guarded']=str_contains($dashboard,"oneid_require_admin_step_up(\$operation, 'ADMIN_ACCESS', false)")&&str_contains($users,"oneid_require_admin_step_up(\$operation, 'ADMIN_ACCESS', false)");$tests['expired_admin_grant_returns_to_user_dashboard']=str_contains($guard,"header('Location: '.APP_URL.'/page/dashboard',true,302)")&&!str_contains($guard,"echo 'Administrator step-up authentication required. Correlation: '");
