...
$checks['Users Access Applications and two Session reports are implemented']=$ready===['executive_summary','users_by_category','access_matrix','access_exceptions','application_readiness','application_acl_coverage','credential_rotation','session_activity','device_summary'];
...
$checks['preview uses allowlisted report keys and nine data providers']=str_contains($preview,'AdminReportReference::resolve')&&str_contains($preview,'admin_report_executive_summary')&&str_contains($preview,'admin_report_users_by_category')&&str_contains($preview,'admin_report_category_access_matrix')&&str_contains($preview,'admin_report_access_exceptions')&&str_contains($preview,'admin_report_application_readiness')&&str_contains($preview,'admin_report_application_acl_coverage')&&str_contains($preview,'admin_report_site_api_credential_rotation')&&str_contains($preview,'admin_report_session_activity')&&str_contains($preview,'admin_report_device_summary');
$checks['database provides the nine read-only report queries']=str_contains($database,'function admin_report_executive_summary')&&str_contains($database,'function admin_report_users_by_category')&&str_contains($database,'function admin_report_category_access_matrix')&&str_contains($database,'function admin_report_access_exceptions')&&str_contains($database,'function admin_report_application_readiness')&&str_contains($database,'function admin_report_application_acl_coverage')&&str_contains($database,'function admin_report_site_api_credential_rotation')&&str_contains($database,'function admin_report_session_activity')&&str_contains($database,'function admin_report_device_summary');
...
$checks['application reports allocate a compact number column and wider application column']=str_contains($preview,'.table-wrap col:first-child{width:4%}')&&str_contains($preview,'.table-wrap col:nth-child(2){width:27%}');
