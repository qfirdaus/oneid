...
$ok(str_contains($q,'filter_var((string)$uid_result[\'data5\'],FILTER_VALIDATE_EMAIL)')&&str_contains($q,'otp_invalidate_active($uid_result[\'u_id\'])')&&str_contains($q,'$auditType = 35')&&str_contains($q,'outcome=%s')&&str_contains($q,'correlation=%s'),'undeliverable recovery fails closed and records correlated outcome');
$login=(string)file_get_contents($root.'/index.php');$ssoFlow=(string)file_get_contents($root.'/lib/SSO_IDP_INC.php');
$ok(str_contains($q,'$mail->Timeout = 10')&&!str_contains($q,'$mail->Timelimit')&&str_contains($q,'session_write_close()'),'SMTP is bounded without dynamic-property deprecation and releases the session lock');
$ok(str_contains($login,'timeout: 20000')&&str_contains($login,'complete: finishRecoveryLoading')&&str_contains($login,'recoveryRequestInFlight'),'recovery UI has timeout, cleanup and double-submit protection');
$ok(str_contains($ssoFlow,'CURLOPT_CONNECTTIMEOUT, 5')&&str_contains($ssoFlow,'CURLOPT_TIMEOUT, 12'),'internal SSO request has bounded timeouts');
$sweetAlertScript='vendors/bower_components/sweetalert/dist/sweetalert.min.js';$notificationScript='assetsM/js/oneid-notifications.js';
$ok(str_contains($q,'AUTH_PASSWORD_REQUIRED')&&str_contains($login,'showLoginInlineError(pilotI18n.loginRequiredUser)')&&str_contains($login,'showLoginInlineError(pilotI18n.loginRequiredPassword)')&&str_contains($login,"response['login_response_msg'] || pilotI18n.loginInvalid"),'login keeps explicit localized validation with stable catalogue keys');
$ok(str_contains($login,'oneidPublicAlert(pilotI18n.requestAccepted')&&str_contains($login,"swal({")&&str_contains($login,$sweetAlertScript)&&strpos($login,$sweetAlertScript)<strpos($login,$notificationScript),'Password Recovery loads and uses localized real SweetAlert feedback');
$ok(str_contains($ui,'id="configuration_recovery"')&&str_contains($ui,'test_password_recovery_email')&&!str_contains($sso,'sso_settings_OTP_email'),'UI, endpoint and service separate recovery from SSO policy');
$stepUp=(string)file_get_contents($root.'/page/admin_step_up.php');
$returnContext=(string)file_get_contents($root.'/app/Auth/AdminStepUpReturnContext.php');
$ok(str_contains($ui,"code==='STEP_UP_REQUIRED'")&&str_contains($ui,'return=configuration_account_recovery')&&str_contains($ui,"context==='configuration_account_recovery'")&&str_contains($returnContext,"'configuration_account_recovery'")&&str_contains($stepUp,'location.replace(successRedirect)'),'recovery test handles security step-up and returns to the correct configuration tab');
