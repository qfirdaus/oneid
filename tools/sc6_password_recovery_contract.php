...
$ok(str_contains($q,'AUTH_PASSWORD_REQUIRED')&&str_contains($login,"showLoginInlineError('Sila masukkan ID Pengguna.'")&&str_contains($login,"showLoginInlineError('Sila masukkan Kata Laluan.'")&&str_contains($login,"response['login_response_msg'] || 'ID Pengguna atau Kata Laluan tidak sah.'"),'login keeps explicit validation with original inline alert feedback');
$ok(str_contains($login,"oneidPublicAlert('Permintaan diterima'")&&str_contains($login,"swal({")&&str_contains($login,$sweetAlertScript)&&strpos($login,$sweetAlertScript)<strpos($login,$notificationScript),'Password Recovery loads and uses real SweetAlert feedback');
