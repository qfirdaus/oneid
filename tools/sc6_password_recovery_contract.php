...
$ok(str_contains($q,'AUTH_PASSWORD_REQUIRED')&&str_contains($login,"oneidPublicAlert('Log masuk tidak berjaya', 'Sila masukkan Kata Laluan.'")&&str_contains($login,"response['login_response_msg'] || 'ID Pengguna atau Kata Laluan tidak sah.'")&&str_contains($login,"swal({")&&str_contains($login,$sweetAlertScript)&&strpos($login,$sweetAlertScript)<strpos($login,$notificationScript),'empty password and authentication failures load and use real SweetAlert feedback');
