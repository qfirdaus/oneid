...
$report(str_contains($ui,'oneid-site-api-rotation.css?v=20260812-6'),'rotation dialog stylesheet uses the current cache-busting version');
...
$report(str_contains($appModalCss,'.oneid-app-info-footer')&&str_contains($appModalCss,'justify-content:flex-end')&&str_contains($ui,'oneid-web-app-modal.css?v=20260814-7'),'App Info footer actions are right-aligned with refreshed styling');
$report(str_contains($ui,'Secure HTTPS URL used in staging')&&str_contains($ui,'oneid-app-card--access')&&str_contains($appModalCss,'.oneid-direct-link-setting__control input{height:1px;opacity:0'),'App Info explains HTTPS-only URLs and keeps Direct Link checkbox inside its card');
$report(str_contains($ui,'oneid-app-card--access')&&str_contains($ui,'oneid-direct-link-setting__switch')&&str_contains($appModalCss,'input:checked~.oneid-direct-link-setting__switch')&&str_contains($ui,'oneid-web-app-modal.css?v=20260814-7'),'Direct Link uses a structured access card and styled toggle switch');
...
$report(str_contains($ui,"$(document).off('focusin.bs.modal')")&&str_contains($ui,'instance.enforceFocus()'),'rotation dialog suspends and restores the Bootstrap parent-modal focus trap for the Other reason textarea');
$report(str_contains($rotationCss,'.oneid-site-api-alert .sa-icon{height:80px')&&str_contains($rotationCss,'width:80px'),'Site API warning and success animations retain the native SweetAlert icon geometry');
$report(str_contains($rotationCss,'.oneid-site-api-alert--result{width:700px')&&str_contains($rotationCss,'.site-api-code-result__feedback{white-space:nowrap}'),'generated credential alert is wide enough for single-line code and copied feedback');
$report(str_contains($rotationCss,'.oneid-site-api-alert,.sweet-alert.oneid-site-api-alert--result{box-sizing:border-box;margin-left:-350px!important')&&str_contains($rotationCss,'.oneid-site-api-alert>p{box-sizing:border-box;max-width:none;width:100%}'),'reason and generated credential alerts share a wide full-content layout');
