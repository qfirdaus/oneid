...
$report(ONEID_APP_VERSION === '2.4.4', 'central application version is 2.4.4');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.4.4',
...
$report(
    str_contains($adminDashboard, 'version: <?php echo json_encode(ONEID_APP_VERSION); ?>')
        && str_contains($adminDashboard, 'tujuh kolum kepada empat')
        && str_contains($adminDashboard, 'top-left alignment')
        && str_contains($adminDashboard, 'Active Sessions'),
    'latest admin release card reads shared v2.4.4 metadata and compact Audit History notes'
);
$expectedHistory = [
...
$report(
    ...
    'release history preserves normalized v2.4.3 through v2.0.0 in order'
);
