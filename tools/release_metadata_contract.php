...
$report(ONEID_APP_VERSION === '2.9.4', 'central application version is 2.9.4');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.9.4',
...
$report(
    ($approvedEnglish[0]['version'] ?? null) === '2.9.4'
        && str_starts_with((string)($approvedEnglish[0]['changes'][0] ?? ''), 'The new Maintenance Mode'),
    'approved English release catalogue is bound to v2.9.4 content'
);
...
$report(
    ...
    'latest admin release card reads shared v2.9.4 bilingual metadata'
);
$expectedHistory = [
...
$report(
    $historyValid
        && count($plainCatalogue) === 54,
    'release history preserves all 54 releases in order'
);
...
$report(
    array_column(array_slice($plainCatalogue, 0, 5), 'version') === ['2.9.4', '2.9.3', '2.9.2', '2.9.1', '2.9.0'],
    'v2.9.4 follows v2.9.3 according to the patch-cap policy'
);
$report(
