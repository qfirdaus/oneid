...
$report(ONEID_APP_VERSION === '2.10.0', 'central application version is 2.10.0');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.10.0',
...
$report(
    ($approvedEnglish[0]['version'] ?? null) === '2.10.0'
        && str_starts_with((string)($approvedEnglish[0]['changes'][0] ?? ''), 'Production application readiness'),
    'approved English release catalogue is bound to v2.10.0 content'
);
...
$report(
    ...
    'latest admin release card reads shared v2.10.0 bilingual metadata'
);
$expectedHistory = [
...
$report(
    $historyValid
        && count($plainCatalogue) === 55,
    'release history preserves all 55 releases in order'
);
...
$report(
    array_column(array_slice($plainCatalogue, 0, 5), 'version') === ['2.10.0', '2.9.4', '2.9.3', '2.9.2', '2.9.1'],
    'v2.10.0 follows v2.9.4 according to the patch-cap rollover policy'
);
$report(
