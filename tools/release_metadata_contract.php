...
$report(ONEID_APP_VERSION === '2.8.2', 'central application version is 2.8.2');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.8.2',
...
$report(
    ($approvedEnglish[0]['version'] ?? null) === '2.8.2'
        && str_starts_with((string)($approvedEnglish[0]['changes'][0] ?? ''), 'The user portal PHP idle timeout'),
    'approved English release catalogue is bound to v2.8.2 content'
);
...
$report(
    ...
    'latest admin release card reads shared v2.8.2 bilingual metadata'
);
$expectedHistory = [
...
$report(
    $historyValid
        && count($plainCatalogue) === 47,
    'release history preserves all 47 releases in order'
);
...
$report(
    array_column(array_slice($plainCatalogue, 0, 2), 'version') === ['2.8.2', '2.8.1'],
    'v2.8.2 is the exact successor to v2.8.1'
);
$report(
