...
$report(ONEID_APP_VERSION === '2.11.0', 'central application version is 2.11.0');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.11.0',
...
$report(
    ($approvedEnglish[0]['version'] ?? null) === '2.11.0'
        && str_starts_with((string)($approvedEnglish[0]['changes'][0] ?? ''), 'Admin Email Notification'),
    'approved English release catalogue is bound to v2.11.0 content'
);
...
$report(
    ...
    'latest admin release card reads shared v2.11.0 bilingual metadata'
);
$expectedHistory = [
...
$report(
    $historyValid
        && count($plainCatalogue) === 60,
    'release history preserves all 60 releases in order'
);
...
$report(
    array_column(array_slice($plainCatalogue, 0, 6), 'version') === ['2.11.0', '2.10.4', '2.10.3', '2.10.2', '2.10.1', '2.10.0'],
    'v2.11.0 follows v2.10.4 according to the patch-cap policy'
);
$report(
