...
$report(ONEID_APP_VERSION === '2.0.10', 'central application version is 2.0.10');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.0.10',
...
$report(
    str_contains($adminDashboard, 'version: <?php echo json_encode(ONEID_APP_VERSION); ?>')
        ...
        && str_contains($adminDashboard, '64-character plan hash')
        && str_contains($adminDashboard, 'isolated restore 18 jadual'),
    'latest admin release card reads shared v2.0.10 metadata and notes'
);
...
$report(
    ...
    'release history preserves v2.0.9 through v2.0.5 in order'
);
$report(
