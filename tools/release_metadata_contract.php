...
$report(ONEID_APP_VERSION === '2.12.1', 'central application version is 2.12.1');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.12.1',
...
$report(
    ($approvedEnglish[0]['version'] ?? null) === '2.12.1'
        && str_starts_with((string)($approvedEnglish[0]['changes'][0] ?? ''), 'Production User MFA changes'),
    'approved English release catalogue is bound to v2.12.1 content'
);
...
$report(
    ...
    'latest admin release card reads shared v2.12.1 bilingual metadata'
);
$expectedHistory = [
...
$report(
    $historyValid
        && count($plainCatalogue) === 63,
    'release history preserves all 63 releases in order'
);
...
$report(
    array_column(array_slice($plainCatalogue, 0, 6), 'version') === ['2.12.1', '2.12.0', '2.11.1', '2.11.0', '2.10.4', '2.10.3'],
    'v2.12.1 follows v2.12.0 according to the patch-cap policy'
);
$report(
