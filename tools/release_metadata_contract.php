...
$report(ONEID_APP_VERSION === '2.5.2', 'central application version is 2.5.2');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.5.2',
...
$report(
    str_contains($adminDashboard, 'version: <?php echo json_encode(ONEID_APP_VERSION); ?>')
        && str_contains($adminDashboard, 'Menu Back to My Account')
        && str_contains($adminDashboard, 'class="pill-yellow"')
        && str_contains($adminDashboard, 'authorization'),
    'latest admin release card reads shared v2.5.2 metadata and highlighted account navigation notes'
);
$expectedHistory = [
    '2.5.1','2.5.0',
...
$report(
    ...
    'release history preserves normalized v2.5.1 through v2.0.0 in order'
);
