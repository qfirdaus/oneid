...
$report(ONEID_APP_VERSION === '2.14.1', 'central application version is 2.14.1');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.14.1',
...
$report(
    ($approvedEnglish[0]['version'] ?? null) === '2.14.1'
        && str_starts_with((string)($approvedEnglish[0]['changes'][0] ?? ''), 'User Account search now shows active accounts'),
    'approved English release catalogue is bound to v2.14.1 content'
);
...
$report(
    ...
    'latest admin release card reads shared v2.14.1 bilingual metadata'
);
$expectedHistory = [
...
$report(
    $historyValid
        && count($plainCatalogue) === 73,
    'release history preserves all 73 releases in order'
);
...
$report(
    array_column(array_slice($plainCatalogue, 0, 6), 'version') === ['2.14.1', '2.14.0', '2.13.4', '2.13.3', '2.13.2', '2.13.1'],
    'v2.14.1 follows v2.14.0 according to the patch-cap policy'
);
$report(
