...
$report(ONEID_APP_VERSION === '2.6.0', 'central application version is 2.6.0');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.6.0',
...
$report(
    str_contains($adminDashboard, 'version: <?php echo json_encode(ONEID_APP_VERSION); ?>')
        && str_contains($adminDashboard, 'External Sync kini dipisahkan')
        && str_contains($adminDashboard, 'Staff dan Undergraduate menggunakan Preview')
        && str_contains($adminDashboard, 'ODL External Sync kekal Read Only Shadow Preview'),
    'latest admin release card reads shared v2.6.0 metadata and source-scoped sync notes'
);
$expectedHistory = [
...
$report(
    ...
    'release history preserves normalized v2.5.4 through v2.0.0 in order'
);
