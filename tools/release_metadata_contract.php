...
$report(ONEID_APP_VERSION === '2.13.1', 'central application version is 2.13.1');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.13.1',
...
$report(
    ($approvedEnglish[0]['version'] ?? null) === '2.13.1'
        && str_starts_with((string)($approvedEnglish[0]['changes'][0] ?? ''), 'Login Banner changes requiring'),
    'approved English release catalogue is bound to v2.13.1 content'
);
...
$report(
    ($latestPlainRelease['version'] ?? null) === ONEID_APP_VERSION
        && count($latestPlainRelease['bm'] ?? []) === 4
        && count($latestPlainRelease['en'] ?? []) === 4,
    'latest admin release card reads shared v2.13.1 bilingual metadata'
);
$expectedHistory = [
...
$report(
    $historyValid
        && count($plainCatalogue) === 68,
    'release history preserves all 68 releases in order'
);
...
$report(
    array_column(array_slice($plainCatalogue, 0, 6), 'version') === ['2.13.1', '2.13.0', '2.12.4', '2.12.3', '2.12.2', '2.12.1'],
    'v2.13.1 follows v2.13.0 according to the patch-cap policy'
);
$report(
