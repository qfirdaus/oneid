...
$report(ONEID_APP_VERSION === '2.12.0', 'central application version is 2.12.0');
$report(
    oneid_application_footer() === '2026 © PTMK | Aplikasi Digital. Version 2.12.0',
...
$report(
    ($approvedEnglish[0]['version'] ?? null) === '2.12.0'
        && str_starts_with((string)($approvedEnglish[0]['changes'][0] ?? ''), 'User Login MFA now supports'),
    'approved English release catalogue is bound to v2.12.0 content'
);
...
$report(
    ...
    'latest admin release card reads shared v2.12.0 bilingual metadata'
);
$expectedHistory = [
...
$report(
    $historyValid
        && count($plainCatalogue) === 62,
    'release history preserves all 62 releases in order'
);
...
$report(
    array_column(array_slice($plainCatalogue, 0, 6), 'version') === ['2.12.0', '2.11.1', '2.11.0', '2.10.4', '2.10.3', '2.10.2'],
    'v2.12.0 follows v2.11.1 as an approved feature release'
);
$report(
