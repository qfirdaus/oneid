...
$report(
    str_contains($ui, "oneid_translate('admin.sync.odl_title')")
        && str_contains($ui, "oneid_translate('admin.sync.ug_title')")
        && str_contains($ui, "oneid_translate('admin.sync.staff_title')")
        && str_contains($ui, "oneid_translate('admin.sync.summary_title')")
        ...
        && str_contains($ui, "oneid_translate('admin.sync.generating')")
        && str_contains($ui, "oneid_translate('admin.sync.summary_boundary')")
        && str_contains($ui, 'refresh_external_sync_notifications()')
        && str_contains($ui, 'response.sync_action_counts || {}')
        && str_contains($ui, "oneid_translate('admin.sync.changes_review')")
        && str_contains($ui, 'external-action-notice')
        && str_contains($ui, 'fa-exclamation-triangle')
        && str_contains($ui, 'modal_odl_shadow_preview')
        && str_contains($ui, "oneid_translate('admin.sync.technical')")
