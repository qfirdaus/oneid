...
$report(str_contains($ui,"sessionAttribute(userName+' ('+userId+')')")&&str_contains($ui,"admin.sessions.user_id")&&str_contains($ui,"active-session-user-copy")&&str_contains($ui,"statusTitle += ' - '"),'UI shows escaped user identity clearly and retains revocation detail in hover titles');
...
$report($code===0&&in_array('RESULT checks=9 failed=0',$output,true),'read-only Active Sessions characterization passes');
