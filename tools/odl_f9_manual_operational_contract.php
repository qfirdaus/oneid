...
$r(str_contains($runtime,"'ONEID_ODL_OPERATIONAL_PREVIEW_ENABLED' => 'false'")
 ...
 &&str_contains($runtime,"'ONEID_ODL_MANUAL_OPERATIONAL_ENABLED' => 'false'"),
...
$r(str_contains($config,"['staging', 'uat']")
 ...
 &&str_contains($config,'$this->onDemandEnabled || $this->manualOperationalEnabled'),
 'on-demand authorization is explicitly limited to staging and UAT');
$r(str_contains($config,'ODL_MANUAL_OPERATIONAL_ENVIRONMENT_INVALID')
 &&str_contains($config,"['staging', 'uat', 'production']")
 &&str_contains($config,'manualOperationalEnabled'),
 'production manual ODL uses an explicit shared Operational approval gate');
...
$r($c===0&&in_array('RESULT checks=16 failed=0',$o,true),'F9 characterization passes');
