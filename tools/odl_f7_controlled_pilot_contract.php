...
'tools/odl_f7_pilot_preview.php','tools/odl_f7_pilot_apply.php'
];
...
$apply=(string)file_get_contents($root.'/tools/odl_f7_pilot_apply.php');
$r(str_contains($apply,'ODL_F7_EXECUTION_AUTHORIZATION_REQUIRED')
    &&str_contains($apply,'ODL_F7_OUTSIDE_CHANGE_WINDOW')
    &&str_contains($apply,'EXPECTED_PLAN_HASH')
    &&str_contains($apply,'EXPECTED_PREVIEW_DIGEST'),
    'one-shot Apply binds authorization window and approved preview');
$r(str_contains($apply,'OdlF7ProcessApprovalStore')
    &&str_contains($apply,'applyApproved')
    &&str_contains($apply,'$freshPlan'),
    'one-shot Apply uses process-local approval and guarded writer');
