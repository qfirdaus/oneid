...
$maintenanceView=(string)file_get_contents($root.'/resources/views/maintenance.php');
$checks['administrator maintenance login is reachable from root and public document roots']=$login!==false&&is_file($root.'/admin/login.php')&&is_file($root.'/public/admin/login.php')&&str_contains((string)file_get_contents($root.'/admin/login.php'),'ONEID_ADMIN_MAINTENANCE_LOGIN')&&str_contains($maintenanceView,"APP_URL . '/admin/login.php'");
$checks['maintenance page uses a wide branded OneID and UPNM presentation']=str_contains($maintenanceView,'max-width:820px')&&str_contains($maintenanceView,"/img/logo_oneid.png")&&str_contains($maintenanceView,"/img/logo_upnm_30.png")&&str_contains($maintenanceView,'maintenance-message');
