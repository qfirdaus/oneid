...
$checks = [
    ...
    'SSO controls explain direct-link semantics' => substr_count($dashboard, 'Open the application directly without the OneID SSO redirect flow.') >= 2,
    ...
    'icon rejection and atomic success are explicitly represented' => str_contains((string) file_get_contents($root . '/app/Admin/WebAppService.php'), 'WA3_ICON_REJECTED') && str_contains((string) file_get_contents($root . '/app/Admin/WebAppService.php'), 'WA4_APP_CREATED_ENVIRONMENT_ASSET'),
    'Add App uses the wide Application Details layout' => str_contains($dashboard, 'id="modal_add_new_app"') && substr_count($dashboard, 'modal-lg oneid-app-info-dialog') >= 2 && str_contains($dashboard, 'oneid-add-app-grid'),
    'created application exposes a professional copyable credential result' => str_contains($dashboard, 'function show_created_app_credential') && str_contains($dashboard, 'copy_created_site_api_code') && str_contains($dashboard, "title:'Application Created'"),
];
