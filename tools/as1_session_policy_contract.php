...
$heartbeatPolicy=$source['tests/characterization/as1_idle_heartbeat_policy.php'];
$report(str_contains($heartbeatPolicy,'1800')&&str_contains($heartbeatPolicy,'28800'),'PHP idle and absolute timeout boundaries remain explicit');
...
$report(str_contains($tool,"--scheduled")
    &&str_contains($tool,"ONEID_SESSION_HOUSEKEEPING_SCHEDULED_ENABLED")
    &&str_contains($tool,"AS1_SCHEDULED_DISABLED"),
    'scheduled housekeeping is non-interactive and deployment-gated');
...
$report(str_contains($tool,"NATURAL_EXPIRED")&&str_contains($tool,"POLICY_REVOKED")&&str_contains($tool,"INVALID_FUTURE")&&str_contains($tool,'ended_at=COALESCE'),
    'housekeeping preserves terminal session history metadata');
...
$report($code1===0&&in_array('RESULT checks=11 failed=0',$out1,true),'idle heartbeat characterization passes');
