    ...
    //Check admin login
     ...
     //First Time Login Check Password had changed or not
      if(isset( $_POST['check_default_password'])){
        ...
        $array = array();
        if(md5($user_info['data4']) == $user_info['u_password']){
     ...
     //Admin
      ...
      if(isset( $_POST['action_add_new_app'])){
        ...
        $results = $operation->action_add_new_app($sp_id,$_POST['add_new_app_name'],$_POST['add_new_app_desc'],str_replace(':/','://', trim(preg_replace('/\/+/', '/',$_POST['add_new_app_url']), '/')),'',$_POST['add_new_app_category'],$checkbox_status);
      ...
      if(isset( $_POST['action_edit_app_info'])){
        ...
        $results = $operation->action_edit_app_info($_POST['edit_app_id'],$_POST['edit_app_name'],$_POST['edit_app_desc'],str_replace(':/','://',trim(preg_replace('/\/+/', '/', $_POST['edit_app_url']), '/')),'',$_POST['edit_app_category'],$checkbox_status);
      ...
      if(isset( $_POST['admin_add_sync_user'])){
            //Create external header
			//echo "hi1";
            $header_id = $operation->action_add_new_ext_header(0);

            //------ 2. Script for Sample direct Functio call from external_data_source_API.php
            $list = EXTERNAL_DATA_SOURCE_GET_ALL_USER();
            //$list = SAMPLE_DATA_SOURCE_GET_ALL_USER();
            //----- END of Direct function Call
/* echo json_encode($list);
return; */
        
            if(!empty($list)){
              //upload data to external temp body
                ...
                if(isset($list[$i]['data4'])){
                  if($list[$i]['data4'] == "" || $list[$i]['data4'] == " "){
                    ...
                    continue;
                      ...
                      // $list[$i]['ext_body_id'] = $operation->action_add_external_temp_body($header_id,$list[$i]['data1'],$list[$i]['data2'],$list[$i]['data3'],$list[$i]['data4'],$list[$i]['data5'],$list[$i]['data6'],$list[$i]['data7'],$list[$i]['data8'],$list[$i]['data9'],$list[$i]['data10'],$list[$i]['data11'],$list[$i]['data12']);
                    }
                  }else{
                    unset($list[$i]);
                    continue;
              ...
              //-----Check for duplicates (staff+pelajar)

             /*  $new_array = array();
              $exists_array    = array();
              $test_array    = array();
              foreach( $list as $element ) {
                  if( !in_array( $element['data4'], $exists_array )) {
                      $exists_array[]    = $element['data4'];
                      $test_array[] = $element;
                  }
                  else{
                    $treshold = 0;
                    $compare_array1 = array();
                    $compare_array2 = $element;

                    foreach( $test_array as $i => $ii ) {
                      if($element['data4'] == $test_array[$i]['data4']){
                        $compare_array1 = $test_array[$i];
                        if($test_array[$i]['ext_data_source_category'] == "Pentadbiran" || $test_array[$i]['ext_data_source_category'] == "Akademik"){
                          $test_array[$i]['ext_data_source_category'] = $test_array[$i]['ext_data_source_category'].$compare_array2['ext_data_source_category'];
                          $test_array[$i]['data8'] = $compare_array2['data2'];
                        }else{
                          $test_array[$i] = $compare_array2;
                          $test_array[$i]['ext_data_source_category'] = $test_array[$i]['ext_data_source_category'].$compare_array1['ext_data_source_category'];
                          $test_array[$i]['data8'] = $compare_array1['data2'];
                        }
                      }
                    }
                  }
              } */
              // echo json_encode($exists_array);
             /*  echo json_encode($test_array);
              return */;
              //-----End of checking

              //$list = array_values($test_array);

              //Update temp header for completing upload to temp table
              $operation->admin_update_ext_header_status($header_id,1,'ext_head_initial_sourcedata',count($list));
              //Generate sha256 hash based on data from external source
              foreach ($list as $i => $ii) {  
                $list[$i]['u_changes_hash'] = hash('sha256',$list[$i]['data1'].$list[$i]['data2'].$list[$i]['data3'].$list[$i]['data4'].$list[$i]['data5'].$list[$i]['data6'].$list[$i]['data7'].$list[$i]['data8'].$list[$i]['data9'].$list[$i]['data10'].$list[$i]['data11'].$list[$i]['data12'].$list[$i]['ext_data_source_category']);
                $list[$i]['source'] = '2';
              }
              //Get SSO User list - Filter A
              $sso_user_list = $operation->sync_get_all_sso_user();

              //Merge Filter A data & external source
              $merged_data = array_merge($list,$sso_user_list);
              // echo json_encode($merged_data);
              // return;

              $filtered_data_A = sync_remove_duplicateKeys($merged_data); //filter/ remove no changes data
              // echo json_encode($filtered_data_A);
              // return;
              ...
              ////echo json_encode($filtered_data_B);
              //return;
              if(!empty($filtered_data_B)){                
                //Now add to SSO user table
                foreach ($filtered_data_B as $i => $ii) {    
                  //determine category
                  $user_category = 0;//default is 0 pending
				  $password = "";
                  ...
                  // echo     $filtered_data_B[$i]['ext_data_source_category'];
                  // echo $user_category;
                  $operation->action_add_new_user_from_external_source($filtered_data_B[$i]['data4'],$user_category,$password,$filtered_data_B[$i]['data1'],$filtered_data_B[$i]['data2'],$filtered_data_B[$i]['data3'],$filtered_data_B[$i]['data4'],$filtered_data_B[$i]['data5'],$filtered_data_B[$i]['data6'],$filtered_data_B[$i]['data7'],$filtered_data_B[$i]['data8'],$filtered_data_B[$i]['data9'],$filtered_data_B[$i]['data10'],$filtered_data_B[$i]['data11'],$filtered_data_B[$i]['data12'],hash('sha256',$filtered_data_B[$i]['data1'].$filtered_data_B[$i]['data2'].$filtered_data_B[$i]['data3'].$filtered_data_B[$i]['data4'].$filtered_data_B[$i]['data5'].$filtered_data_B[$i]['data6'].$filtered_data_B[$i]['data7'].$filtered_data_B[$i]['data8'].$filtered_data_B[$i]['data9'].$filtered_data_B[$i]['data10'].$filtered_data_B[$i]['data11'].$filtered_data_B[$i]['data12'].$filtered_data_B[$i]['ext_data_source_category']));
            ...
            //Get final header output
            $header_info = $operation->action_get_ext_header($header_id);
            $operation->syslog_record(22,$_SESSION['login_user'],getUserIP());
      ...
      if(isset( $_POST['remove_acl_category'])){
        ...
        $operation->syslog_record(29,$_SESSION['login_user']." -> ".$_POST['uc_id']." -> ".$_POST['sp_id'],getUserIP());
        echo json_encode($results);
      }
      ...
      if(isset( $_POST['admin_change_user_category'])){
        ...
        if(!empty($user_info)){          
          $results = $operation->admin_change_user_category($_POST['user_id'],$_POST['category_id']);
          echo json_encode($results);
        }else{
        ...
        if(isset( $_POST['forgot_password_id'])){
          ...
          if(!empty($uid_result)){
            ...
            if(!empty($otp_search_result)){
              ...
              if($update_result==1){
                echo json_encode(array( 'result' => "true",'msg' => "Check your email. Use the last OTP sent."));
                ...
                echo json_encode(array( 'result' => "true",'msg' => "OTP Sent to your email. Check your inbox."));
                $operation->syslog_record(9,"User: ".$uid_result['data1']." -> OTP Forgot Password Created",getUserIP());
              }
            }else{
    ...
    function OTP_EMAIL_Sender($otp_code,$email,$user_name){
      ...
      $html_body_header = 'Password Reset Request';
      $html_body_content = 'We received a request to reset your password for your SSO account. Use the OTP below to proceed:';
      ...
      <html>
        ...
        <body style="font-family: Arial, sans-serif; margin: 0; padding: 0; background-color: #f4f4f4;">
          <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f4f4f4; padding: 20px;">
            ...
            $mail->Subject = 'OTP For UPNM SSO - Reset Password';
