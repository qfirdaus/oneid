         ...
         $('#modal_category_user_list_view').modal('show');
           ...
           $(document).on('click','.archived-app-purge:not(:disabled)',function(){
              ...
              var purgePanel='<div class="oneid-archive-purge-panel">'+
                 ...
                 '<div class="oneid-archive-purge-field"><label for="archive_purge_reason">'+webAppManagementText(adminText('admin.archive.reason_label'))+'</label><small>'+webAppManagementText(adminText('admin.archive.reason_help'))+'</small><textarea id="archive_purge_reason" class="form-control" rows="3" minlength="10" maxlength="500" placeholder="'+webAppManagementAttribute(adminText('admin.archive.reason'))+'"></textarea></div>'+
              '</div>';
              swal({title:adminText('admin.archive.purge_title'),text:purgePanel,html:true,type:'warning',showCancelButton:true,confirmButtonColor:'#d9534f',confirmButtonText:adminText('admin.archive.purge_confirm'),cancelButtonText:adminText('admin.archive.cancel'),closeOnConfirm:false,allowOutsideClick:false},function(){
                 var confirmation=$('#archive_purge_confirmation').val()||'',reason=$('#archive_purge_reason').val()||'';
                 if(confirmation!==appName||String(reason).trim().length<10){swal.showInputError(adminText('admin.archive.validation'));return false;}
                 $.post('../lib/q_func',{admin_purge_archived_app:'',app_id:appId,confirmation:confirmation,reason:reason},function(response){
                    if(Number(response.status)===1){swal(adminText('admin.archive.purged'),adminText('admin.archive.reference')+': '+response.correlation_id,'success');open_archived_webapps();}
                    else swal(adminText('admin.archive.purge_blocked'),'Code: '+(response.code||'WA7_PURGE_FAILED')+'\n'+adminText('admin.archive.reference')+': '+(response.correlation_id||'Unavailable'),'error');
                 },'json');
                 return false;
              });
