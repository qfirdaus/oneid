...
<html lang="<?=htmlspecialchars(oneid_current_locale(), ENT_QUOTES, 'UTF-8')?>">
   ...
   <body class="<?=trim(oneid_environment_body_class())?>">
      ...
      <script>
         ...
         $(document).on('click','#session_view_history',function(){$('.session-view-tabs button').removeClass('is-active').attr('aria-selected','false');$(this).addClass('is-active').attr('aria-selected','true');$('#app_security_session_list').hide();$('#session_history_list').prop('hidden',false);load_session_history(1);});
