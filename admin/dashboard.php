...
<html lang="<?=htmlspecialchars(oneid_current_locale(), ENT_QUOTES, 'UTF-8')?>">
   ...
   <body class="<?=trim(oneid_environment_body_class())?>">
      ...
      <script>
         ...
         function load_session_history(page){initializeSessionHistoryDates();sessionHistoryPage=Number(page||sessionHistoryPage||1);$('#session_history_rows').html('<tr class="active-session-state-row is-loading"><td colspan="4"><span class="active-session-state-icon"><i class="fa fa-circle-o-notch fa-spin"></i></span><strong>'+adminText('admin.sessions.loading_history')+'</strong></td></tr>');$.post('../lib/q_func',{admin_get_session_history:'',page:String(sessionHistoryPage),page_size:String($('#session_history_page_size').val()||'10'),query:String($('#session_history_query').val()||'').trim(),reason:String($('#session_history_reason').val()||'all'),date_from:$('#session_history_from').val(),date_to:$('#session_history_to').val()},function(response){var esc=function(v){return $('<div>').text(v==null?'':String(v)).html();};if(!response||Number(response.status)!==1){$('#session_history_rows').html('<tr class="active-session-state-row is-error"><td colspan="4"><strong>'+esc(adminText('admin.sessions.history_unavailable'))+'</strong><small>'+esc(response&&response.code?response.code:'SH1_HISTORY_FAILED')+'</small></td></tr>');return;}sessionHistoryLoaded=true;var rows='';$.each(response.data||[],function(_,s){var idLabel=s.public_user_id_type==='staff'?adminText('admin.sessions.staff_no'):adminText('admin.sessions.matric_no');rows+='<tr><td><span class="active-session-user-copy"><strong>'+esc(s.name)+(s.is_admin?' <small class="session-history-admin">ADMIN</small>':'')+'</strong><small><span>'+esc(idLabel)+'</span>'+esc(s.public_user_id)+'</small></span></td><td><span class="active-session-timeline"><span><small>'+adminText('admin.sessions.issued')+'</small><time>'+esc(admin_format_datetime(s.issued_at))+'</time></span><span><small>'+adminText('admin.sessions.ended')+'</small><time>'+esc(admin_format_datetime(s.ended_at||s.last_activity_at))+'</time></span></span></td><td><span class="active-session-cell active-session-device"><i class="fa fa-desktop"></i>'+esc(s.device_info)+'</span></td><td><span class="active-session-status is-expired"><i class="fa fa-history"></i><span>'+esc(sessionEndLabel(s.end_reason))+'</span></span></td></tr>';});$('#session_history_rows').html(rows||'<tr class="active-session-state-row"><td colspan="4"><strong>'+adminText('admin.sessions.history_empty')+'</strong></td></tr>');var m=response.meta||{},p=Number(m.page||1),pages=Number(m.total_pages||1);$('#session_history_pagination').html(Number(m.total||0)?'<button type="button" data-history-page="'+(p-1)+'" '+(p<=1?'disabled':'')+'><i class="fa fa-chevron-left"></i></button><span>'+adminText('admin.sessions.page_of',{page:p,pages:pages})+'</span><button type="button" data-history-page="'+(p+1)+'" '+(p>=pages?'disabled':'')+'><i class="fa fa-chevron-right"></i></button>':'');},'json').fail(function(){$('#session_history_rows').html('<tr class="active-session-state-row is-error"><td colspan="4"><strong>'+adminText('admin.sessions.history_unavailable')+'</strong></td></tr>');});}
