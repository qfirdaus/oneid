      ...
      <script>
         ...
         function load_session_history(page){
            ...
            $.post('../lib/q_func',{admin_get_session_history:'',page:String(sessionHistoryPage),page_size:String($('#session_history_page_size').val()||'10'),query:String($('#session_history_query').val()||'').trim(),reason:String($('#session_history_reason').val()||'all'),date_from:$('#session_history_from').val(),date_to:$('#session_history_to').val()},function(response){
               ...
               if(!response||Number(response.status)!==1){$('#session_history_rows').html('<tr class="active-session-state-row is-error"><td colspan="4"><strong>'+esc(adminText('admin.sessions.history_unavailable'))+'</strong><small>'+esc(response&&response.code?response.code:'SH1_HISTORY_FAILED')+'</small></td></tr>');return;}
               ...
               $.each(response.data||[],function(_,s){
                  ...
                  rows+='<tr>';
                  rows+='<td data-label="'+sessionAttribute(adminText('admin.sessions.user'))+'"><span class="active-session-user-copy" data-session-tooltip title="'+sessionAttribute(userTooltip)+'"><strong>'+esc(userName)+(s.is_admin?' <small class="session-history-admin">ADMIN</small>':'')+'</strong><small><span>'+esc(idLabel)+'</span>'+esc(publicUserId)+'</small></span></td>';
                  rows+='<td data-label="'+sessionAttribute(adminText('admin.sessions.activity'))+'"><span class="active-session-timeline session-history-timeline" data-session-tooltip title="'+sessionAttribute(activityTooltip)+'"><span><small>'+adminText('admin.sessions.issued')+'</small><time>'+esc(issuedAt)+'</time></span><span><small>'+adminText('admin.sessions.ended')+'</small><time>'+esc(endedAt)+'</time></span></span></td>';
                  rows+='<td data-label="'+sessionAttribute(adminText('admin.sessions.device'))+'"><span class="active-session-cell active-session-device" data-session-tooltip title="'+sessionAttribute(deviceInfo)+'"><i class="fa fa-desktop" aria-hidden="true"></i><span>'+esc(deviceInfo)+'</span></span></td>';
                  rows+='<td data-label="'+sessionAttribute(adminText('admin.sessions.end_reason'))+'"><span class="active-session-status is-expired" data-session-tooltip title="'+sessionAttribute(endReason)+'"><i class="fa fa-history" aria-hidden="true"></i><span>'+esc(endReason)+'</span></span></td></tr>';
               });
            ...
            },'json').fail(function(){$('#session_history_rows').html('<tr class="active-session-state-row is-error"><td colspan="4"><strong>'+adminText('admin.sessions.history_unavailable')+'</strong></td></tr>');});
         }
