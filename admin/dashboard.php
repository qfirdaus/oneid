...
<html lang="<?=htmlspecialchars(oneid_current_locale(), ENT_QUOTES, 'UTF-8')?>">
   ...
   <body class="<?=trim(oneid_environment_body_class())?>">
      ...
      <?php $maintenanceAdminPolicy=\OneId\App\Maintenance\MaintenancePolicy::evaluate($operation->get_maintenance_config()?:[]); if($maintenanceAdminPolicy['active']): ?><div style="position:fixed;z-index:99999;top:0;left:0;right:0;background:#a94442;color:#fff;text-align:center;padding:7px;font-weight:700">MAINTENANCE MODE ACTIVE — public access is unavailable; administrator access is being audited.</div><?php endif; ?>
      <!--Preloader-->
                                                   ...
                                                   <div class="col-lg-12">
                                                      ...
                                                      <div class="sso-config-panel">
                                                         ...
                                                         <section class="tab-pane fade" id="configuration_maintenance" role="tabpanel" aria-labelledby="configuration_maintenance_tab">
                                                            <div class="sso-config-panel"><div class="sso-config-header"><div><span class="sso-config-eyebrow">SERVICE AVAILABILITY</span><h4 class="sso-config-title">Maintenance Mode</h4><p class="sso-config-intro">Block public OneID access while authorized administrators retain controlled access.</p></div><button class="sso-config-save" id="maintenance_save" type="button" onclick="saveMaintenanceConfiguration();"><i class="fa fa-check"></i> Review &amp; Save</button></div><div class="sso-config-body">
                                                               <div class="sso-config-note sso-config-note-warning"><i class="fa fa-shield"></i><p id="maintenance_status">Loading maintenance status…</p></div>
                                                               <div class="sso-config-row"><div class="sso-config-copy"><span class="sso-config-index">01</span><div><label for="maintenance_mode">Operating mode</label><p>Scheduled mode ends automatically. Until further notice remains active until an administrator disables it.</p></div></div><div class="sso-config-control"><select class="form-control" id="maintenance_mode"><option value="OFF">Normal operation</option><option value="SCHEDULED">Scheduled maintenance</option><option value="INDEFINITE">Until further notice</option></select></div></div>
                                                               <div class="sso-config-row" id="maintenance_time_row"><div class="sso-config-copy"><span class="sso-config-index">02</span><div><label>Maintenance period</label><p>Times are entered in Asia/Kuala_Lumpur.</p></div></div><div class="sso-config-control"><label for="maintenance_starts_at">Starts</label><input class="form-control" type="datetime-local" id="maintenance_starts_at"><label class="mt-10" for="maintenance_ends_at">Ends</label><input class="form-control" type="datetime-local" id="maintenance_ends_at"></div></div>
                                                               <div class="sso-config-row"><div class="sso-config-copy"><span class="sso-config-index">03</span><div><label>Public message</label><p>Provide clear BM and English information without exposing technical details.</p></div></div><div class="sso-config-control"><input class="form-control" id="maintenance_title_ms" maxlength="160" placeholder="Tajuk BM"><textarea class="form-control mt-10" id="maintenance_message_ms" maxlength="1000" rows="3" placeholder="Mesej BM"></textarea><input class="form-control mt-10" id="maintenance_title_en" maxlength="160" placeholder="English title"><textarea class="form-control mt-10" id="maintenance_message_en" maxlength="1000" rows="3" placeholder="English message"></textarea></div></div>
                                                               <div class="sso-config-row"><div class="sso-config-copy"><span class="sso-config-index">04</span><div><label for="maintenance_reason">Change reason</label><p>Required for audit history, minimum 10 characters.</p></div></div><div class="sso-config-control"><textarea class="form-control" id="maintenance_reason" maxlength="500" rows="3"></textarea></div></div>
                                                               <div class="sso-config-note"><i class="fa fa-info-circle"></i><p>During maintenance, use <b>/admin/login</b>. Only active administrator accounts may continue through MFA and Admin Step-Up.</p></div>
                                                            </div></div>
                                                         </section>
      ...
      <script>
         ...
         function loadMaintenanceConfiguration(){$.post('../lib/q_func',{admin_get_maintenance_configuration:''},function(r){if(!r||Number(r.status)!==1){$('#maintenance_status').text('Maintenance configuration is unavailable. Apply the approved schema migration first.');return;}var d=r.data||{};maintenanceConfigurationVersion=Number(d.configuration_version||0);$('#maintenance_mode').val(d.mode||'OFF');$('#maintenance_starts_at').val(maintenanceLocalValue(d.starts_at));$('#maintenance_ends_at').val(maintenanceLocalValue(d.ends_at));$('#maintenance_title_ms').val(d.title_ms||'');$('#maintenance_title_en').val(d.title_en||'');$('#maintenance_message_ms').val(d.message_ms||'');$('#maintenance_message_en').val(d.message_en||'');$('#maintenance_status').text('Effective state: '+String(d.state||'OFF'));toggleMaintenanceTimes();},'json');}
         function toggleMaintenanceTimes(){var mode=$('#maintenance_mode').val();$('#maintenance_time_row').toggle(mode!=='OFF');$('#maintenance_ends_at').prop('disabled',mode!=='SCHEDULED');}
         $('#maintenance_mode').on('change',toggleMaintenanceTimes);
         function saveMaintenanceConfiguration(){var reason=$.trim($('#maintenance_reason').val());if(reason.length<10){swal('Reason required','Enter a reason of at least 10 characters.','warning');return;}var data={admin_update_maintenance_configuration:'',mode:$('#maintenance_mode').val(),starts_at:$('#maintenance_starts_at').val(),ends_at:$('#maintenance_ends_at').val(),title_ms:$('#maintenance_title_ms').val(),title_en:$('#maintenance_title_en').val(),message_ms:$('#maintenance_message_ms').val(),message_en:$('#maintenance_message_en').val(),change_reason:reason,configuration_version:maintenanceConfigurationVersion};var warning=data.mode==='OFF'?'Public access will be restored immediately.':'Public users will be blocked when this policy becomes active. Administrators must use /admin/login.';swal({title:'Review Maintenance Mode',text:'Mode: '+data.mode+'\n'+warning,type:'warning',showCancelButton:true,confirmButtonText:'Apply Maintenance Policy',closeOnConfirm:false},function(ok){if(!ok)return;$.post('../lib/q_func',data,function(r){if(r&&Number(r.status)===1){$('#maintenance_reason').val('');swal('Maintenance policy saved','Reference: '+r.correlation_id,'success');loadMaintenanceConfiguration();}else{swal('Policy not saved','Code: '+(r&&r.code?r.code:'MT3_FAILED'),'error');}},'json').fail(function(xhr){var code=xhr.responseJSON&&xhr.responseJSON.code?xhr.responseJSON.code:'';if(xhr.status===403&&String(code).indexOf('STEP_UP')===0){window.location.href='../page/admin-step-up?purpose=SECURITY_CONFIGURATION_CHANGE&return=configuration_maintenance';}else{swal('Policy not saved','HTTP '+xhr.status,'error');}});});}
