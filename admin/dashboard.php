      ...
      <script>
         ...
         function saveMaintenanceConfiguration(){
            ...
            swal({title:adminText('admin.maintenance.review_title'),text:review,html:true,type:'warning',showCancelButton:true,confirmButtonText:adminText('admin.maintenance.apply'),cancelButtonText:adminText('admin.maintenance.cancel'),closeOnConfirm:false,allowEscapeKey:false,allowOutsideClick:false},function(ok){
               ...
               $.post('../lib/q_func',data,function(r){if(r&&Number(r.status)===1){maintenanceSelectedReason='';$('.maintenance-reason-options button').removeClass('is-selected').attr('aria-checked','false');$('#maintenance_reason').val('');$('#maintenance_other_reason_wrap').prop('hidden',true);swal(adminText('admin.maintenance.saved_title'),adminText('admin.maintenance.saved_body')+' '+r.correlation_id,'success');loadMaintenanceConfiguration();}else{swal(adminText('admin.maintenance.failed_title'),'Code: '+(r&&r.code?r.code:'MT3_FAILED'),'error');}},'json').fail(function(xhr){var code=xhr.responseJSON&&xhr.responseJSON.code?xhr.responseJSON.code:'';if(xhr.status===403&&String(code).indexOf('STEP_UP')===0){window.location.href='../page/admin-step-up?purpose=SECURITY_CONFIGURATION_CHANGE&return=configuration_maintenance';}else{swal(adminText('admin.maintenance.failed_title'),'HTTP '+xhr.status,'error');}});
            });
