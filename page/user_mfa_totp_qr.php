...
if (!$factor) { http_response_code(400); exit; }
$pdo = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
$statement = $pdo->prepare(
    "SELECT encrypted_secret,secret_nonce,key_version
       FROM user_mfa_factors
      WHERE factor_id=:factor AND u_id=:user
        AND factor_status='PENDING'
        AND enrollment_session_hash=:session
        AND enrollment_browser_digest=:browser"
);
$statement->execute([
    ':factor' => $factor,
