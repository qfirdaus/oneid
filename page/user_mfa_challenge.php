...
$maintenanceMfa=$maintenanceAdmin||$maintenanceDeveloper;
...
$policyTable=$maintenanceMfa?'maintenance_mfa_policy':'user_login_mfa_policy';
$statement=$pdo->prepare(
  "SELECT t.transaction_status,t.expires_at,p.email_enabled,p.totp_enabled,
     ...
     JOIN {$policyTable} p ON p.singleton_key=1
...
$maintenanceTotpRuntime=!$maintenanceMfa||filter_var(oneid_config('ONEID_MAINTENANCE_MFA_TOTP_ENABLED','false'),FILTER_VALIDATE_BOOLEAN);
$totp=$maintenanceTotpRuntime&&(int)$state['totp_enabled']===1&&$activeTotp;
...
<title><?=$h('user_mfa.title.challenge')?> | OneID@UPNM</title><link rel="stylesheet" href="../dist/css/user-mfa-flow.css?v=20260831-1"><link rel="stylesheet" href="../dist/css/oneid-environment-banner.css?v=20260810-1"><?php if(!$maintenanceMfa): ?><link rel="stylesheet" href="../dist/css/oneid-accessibility-baseline.css?v=20260915-1"><link rel="stylesheet" href="../dist/css/oneid-display-settings.css?v=20260915-4"><script src="../dist/js/oneid-display-settings.js?v=20260915-4"></script><?php endif; ?></head>
<body class="user-mfa-flow<?=oneid_environment_body_class()?>"><?php oneid_render_environment_banner(); ?><?php if(!$maintenanceMfa): oneid_render_display_settings(); endif; ?><main class="mfa-shell"><aside class="mfa-brand"><img class="mfa-logo" src="../img/logo_oneid-1.png" alt="OneID@UPNM"><div class="mfa-brand-copy"><span class="mfa-eyebrow"><?=$h('stepup.protected_access')?></span><h1><?=$h('user_mfa.title.challenge')?></h1><p><?=$h('user_mfa.challenge.security_intro')?></p></div></aside>
...
<div id="emailBox" class="mfa-factor mfa-hidden"><div class="mfa-factor-heading"><span class="mfa-factor-icon" aria-hidden="true">&#9993;</span><span><strong><?=$h('stepup.email_title')?></strong><small><?=$h('stepup.email_will_send')?> <?=htmlspecialchars($masked,ENT_QUOTES,'UTF-8')?></small></span></div><div class="mfa-email-action"><span class="mfa-step-label"><?=$h('stepup.email_action_label')?></span><strong><?=$h('stepup.email_action_title')?></strong><p><?=$h('stepup.email_action_help')?></p><button id="sendEmail" class="mfa-button mfa-send-email"><span aria-hidden="true">&#9993;</span> <?=$h('stepup.email_send')?> <span aria-hidden="true">&#8594;</span></button></div><div class="mfa-field"><label for="emailCode"><?=$h('stepup.six_digit_code')?></label><input id="emailCode" class="mfa-control mfa-otp" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" autocomplete="one-time-code" placeholder="000000"></div><button id="verifyEmail" class="mfa-button"><?=$h('stepup.email_verify')?></button></div>
...
</section><button id="cancelLogin" type="button" class="mfa-back">&#8592; <?=$h('stepup.back')?></button><footer class="mfa-foot"><span>OneID@UPNM &bull; <?=$h('user_mfa.security.footer')?></span><span><?=$h('user_mfa.security.department')?></span></footer></section></main>
<script>
...
cancelLoginButton.addEventListener('click',async()=>{try{cancelLoginButton.disabled=true;const r=await post('user_mfa_cancel_login');location.replace(r.redirect_uri||<?=json_encode(APP_URL.'/')?>)}catch(e){cancelLoginButton.disabled=false;msg(errorText(e))}});
